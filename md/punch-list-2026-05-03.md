...
- [x] **AWS redirect for hostname canonicalization.** **Resolved as of 2026-05-07.** `https://www.discovercloudcroft.com/` now correctly 301-redirects to `https://discovercloudcroft.com/`, verified via cache-busted curl + fresh-path check (origin response, not stale cache). Path preservation confirmed. The 2026-05-03 interim state ("Option A — accept dual-serving") is superseded; the host-based redirect rule eventually activated, possibly after Amplify-side propagation delay. See [www-redirect-fix-2026-05-03.txt](../www-redirect-fix-2026-05-03.txt) for the full investigation history.
...
- **2026-05-04 to 2026-05-07:** Homepage `<title>` aligned with `og:title`. Live audit confirmed 109 pages serving cleanly with all redirects, og:images, JSON-LD, and meta in place.
- **2026-05-07:** 5 feeder market landing pages shipped (El Paso, Alamogordo, Las Cruces, Lubbock, Ruidoso). Atom 1.0 feed at `/feed.xml` + `sync-feed.py` generator. Editor bios scaffolded at `about/editors/` with template + README publishing playbook. GA4 propagated to 100% of pages via new `sync-analytics.py`. **www → apex 301 redirect verified active**, superseding the 2026-05-03 Option A interim state. 6 of 6 This-quarter items now closed.
